        ...
        if(new_word == 1){                                                      //if it the first space after some letter- and not space after space
            ...
            if(current->PostingList != NULL){                                   //if the posting list exists
                ...
                if(PostingList->First != NULL){                                 //not empty posting list
                    ...
                    while(PostingNode->Next != NULL){                           //search in PList
                        if(strcmp(PostingNode->DocName,DocName) == 0){
                            if(PostingNode->Doc_id == document_id) break;
                            PostingNode->Amount++;                              //increasing the amount of all words from the same file because of the max/min count
                            if(PostingNode->Amount > temp_amount)
                                temp_amount = PostingNode->Amount;
                        }
                    ...
                    if(found == 1){                                             //Plist node for this document already exists
                        ...
                        newnode->DocName = malloc(strlen(DocName)*sizeof(char));
                ...
                ((current->PostingList)->First)->DocName = malloc(strlen(DocName)*sizeof(char));
...
PListNode* maxcount(char* keyword){
    ...
    PListNode *current;
    PListNode *max;
    First = dfSingle(keyword, 0);                                               //calling df single to get the posting list of the word given
    max = First;
    if(First != NULL){
        current = First;
        while(current != NULL){                                                 //search on posting list for the max
            if(max->Amount < current->Amount){
                max = current;
            }else if(max->Amount = current->Amount){
                if(strcmp(max->DocName, current->DocName) >= 0){
                    max = current;
                }
            }
            current = current->Next;
    ...
    if(First != NULL){
        ...
        while(current != NULL){                                                 //search on posting list for the max
            if(min->Amount > current->Amount){
                min = current;
            }else if(min->Amount = current->Amount){
                if(strcmp(min->DocName, current->DocName) >= 0){
                    min = current;
                }
            }
            current = current->Next;
